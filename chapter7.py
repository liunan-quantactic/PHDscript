...
mean=[]
for t in period:
    mean.append(need_df.loc[t].mean(axis=0))
...
for c in all_code:
    ...
    if len(temp_df)>=10:
        codes.append(c)
        y=temp_df['chg']
        x=temp_df[["beta","EBITDA2EV","free_EV","MOMOM","STOM","risk_aver"]]
...
XX=pd.DataFrame(betas,index=codes,columns=["const","beta","EBITDA2EV","free_EV","MOMOM","STOM","risk_aver"])
XXB=XX[["beta","EBITDA2EV","free_EV","MOMOM","STOM","risk_aver"]].values
XXBQ=np.dot(XXB,Q)
